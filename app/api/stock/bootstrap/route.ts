  ...
  await prisma.$transaction(async (tx) => {
    for (const p of INITIAL_PRESETS) {
      await tx.stockPreset.upsert({
        where: { name: p.name },
        update: { category: p.category, unit: p.unit, minQuantity: p.minQuantity, unitCost: p.unitCost },
        create: { name: p.name, category: p.category, unit: p.unit, minQuantity: p.minQuantity, unitCost: p.unitCost },
      });

      const existing = await tx.stockItem.findFirst({ where: { name: p.name } });
      if (!existing) {
